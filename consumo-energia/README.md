# 🔌 Calculadora de Consumo de Energia - Geladeira

![Python](https://img.shields.io/badge/Python-3.9%2B-blue?logo=python&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?logo=github&logoColor=white)
![Energy](https://img.shields.io/badge/Energy-Eficiente-green)
![License](https://img.shields.io/badge/License-MIT-yellow)
![Status](https://img.shields.io/badge/Status-Ativo-brightgreen)

## 📌 Sobre o Projeto

Aplicação em **Python** que calcula o consumo mensal de energia elétrica e o custo
estimado de eletrodomésticos, com foco inicial em geladeiras.

**Fluxo:** Usuário informa potência e horas de uso → programa calcula consumo → exibe custo estimado.

## 📐 Fórmula Utilizada

```
Consumo Mensal (kWh) = (Potência em W × Horas/Dia × 30 dias) ÷ 1000
```

## 🚀 Como Executar

### Pré-requisitos

- Python 3.9 ou superior
- Nenhuma dependência externa

### Passos

1. **Clone o repositório:**
```bash
   git clone https://github.com/seu-usuario/seu-repositorio.git
   cd seu-repositorio
```

2. **Execute o programa:**
```bash
   python app.py
```

3. **Insira os dados solicitados:**
   - Nome do aparelho
   - Potência em watts (W)
   - Tempo médio de uso diário (horas)

4. **Visualize os resultados:**
   - Consumo estimado em kWh/mês
   - Custo estimado mensal

## 📊 Exemplo de Uso

```
Nome do aparelho: Geladeira
Potência (W): 204
Horas de uso por dia: 24

Consumo estimado: 146.88 kWh/mês
Custo estimado: R$ 110.16
```

**📸 Execução do programa no VS Code:**

![Execução do programa](https://github.com/user-attachments/assets/a7b7fc2b-3fb6-438d-aa08-b77b9bb656d1)

## 💡 Tecnologias

- 🐍 Python 3.9+
- 💻 GitHub
- ⚡ Cálculos de Eficiência Energética

## 📝 Licença

Este projeto está sob a licença MIT.
